"""
generate_vector_prompt_vectors.py
==================================
MODIFIED FROM EASYEDIT2 ORIGINAL

Change 1  generate_vector_prompt_vectors: added dataset_name parameter
          (unused, reserved for consistency with other generator signatures).
...
def generate_vector_prompt_vectors(hparams: VectorPromptHyperParams,
                                   dataset, model=None,
                                   dataset_name=None):  # Change 1
    ...
    if args.generate_prompt_params:
        ...
        for item in dataset:
            ...
            if prompt not in cache_memory:
                cache_memory[prompt] = generate_prompt(
                    model, tokenizer, prompt, args.generate_prompt_params
                )
    ...
    pos_tokens_list, neg_tokens_list = get_tokens_for_vector_prompt(
        dataset, tokenizer, hparams
    )
    ...
    try:
        ...
        for layer in args.layers:
            vec = (
                torch.stack(pos_activations[layer])
                - torch.stack(neg_activations[layer])
            ).mean(dim=0)
            if hparams.save_vectors:
                out_dir = os.path.join(args.steer_vector_output_dir, "vector_prompt_vector")
